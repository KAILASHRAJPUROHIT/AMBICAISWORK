...
def _segment_paths_async(paths: list, category: str | None = None) -> None:
    ...
    def _run():
        ...
        try:
            ...
            for path in paths:
                try:
                    result_path, angle = sam_locate.tight_crop(path, path, expect=expect, straighten=False, category=category)
...
def _force_vertical_hang(path: str) -> None:
    """Rotates a saved crop so its elongated axis is vertical, ornamental
    (heavier) end down -- confirmed live (2026-08-19, tag TP22/83): an
    earring pair came out wide/diagonal because neither the single-piece
    straighten path's "nearest small correction" nor the pair-crop path
    (no straighten support at all) force a true hang orientation, only a
    tilt-level one. Requested explicitly: the piece's bottom should face
    where the MAIN VIEW/LEFT ANGLE/RIGHT ANGLE caption sits, i.e. hang
    downward in frame, matching how it's actually worn.

    PCA on the gold-pixel mask, not minAreaRect: this class of piece
    (earrings, jhumka, nath, tikka) is reliably elongated by design (a
    stud/hook plus a dangling chain/bell/plate), unlike the compact
    near-square shapes minAreaRect was chosen for elsewhere in this file
    (see sam_locate._upright_angle's docstring) -- PCA's principal axis is
    stable exactly when the shape is genuinely elongated, and unstable
    exactly where minAreaRect is used instead.
    ...
    chain terminus vs. a plain post) -- if the top half has more mass
    than the bottom after aligning to vertical, the piece is upside down
    and gets flipped 180. A real, testable heuristic for this dataset,
    not a universal guarantee -- fails open (leaves the crop as-is) on
    any error, same convention as every other step in this pipeline.
    """
    log = logging.getLogger("capture_tool")
    try:
        bgr = cv2.imread(path)
        ...
        # Flip check: more mass on top than bottom means it's upside down.
        ...
        log.info("_force_vertical_hang done for %s (correction=%.1f, flipped=%s)",
                 path, correction, top_mass > bottom_mass)
    except Exception:
        log.exception("_force_vertical_hang FAILED for %s -- leaving crop as-is", path)
