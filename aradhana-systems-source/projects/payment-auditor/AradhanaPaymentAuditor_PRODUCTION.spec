...
datas = [('backend', 'backend'), ('launcher_config.json', '.'), ('.env.example', '.'), ('frontend/dist', 'frontend/dist')]
binaries = []
hiddenimports = ['uvicorn.logging', 'uvicorn.protocols', 'uvicorn.protocols.http', 'uvicorn.protocols.http.auto', 'uvicorn.protocols.websockets', 'uvicorn.protocols.websockets.auto', 'uvicorn.lifespan', 'uvicorn.lifespan.on', 'imaplib', 'ssl', 'socket', 'email', 'email.message', 'email.header', 'email.utils', 'email.parser', 'email.policy', 'sqlite3', 'pathlib', 'threading', 'multiprocessing', 'subprocess', 'signal', 'quopri', 'base64', 'datetime', 'decimal']
...
datas += tmp_ret[0]; binaries += tmp_ret[1]; hiddenimports += tmp_ret[2]
tmp_ret = collect_all('typing_inspection')
datas += tmp_ret[0]; binaries += tmp_ret[1]; hiddenimports += tmp_ret[2]
...
exe = EXE(
    ...
    a.datas,
    [],
    name='AradhanaPaymentAuditor_PRODUCTION',
    debug=False,
