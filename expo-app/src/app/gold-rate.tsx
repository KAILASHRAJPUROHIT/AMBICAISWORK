...
export default function GoldRateScreen() {
  ...
  const rateChange = g ? g.high - g.low : 0;
  const rateChangePercent = g ? ((rateChange / g.low) * 100).toFixed(2) : '0';

  const shareRates = () => {
    if (!g) return;
    const msg = [
      `Today's Gold & Silver Rates — ${MASTER.displayName}, Boisar`,
      ``,
        ...
        <ScrollView
          ...
          <View style={styles.headerRow}>
            <View>
              ...
              <ThemedText style={styles.screenSub}>{MASTER.displayName}, Boisar</ThemedText>
            </View>
            ...
            <>
              ...
              <View style={styles.heroCard}>
                ...
                <View style={styles.rangeContainer}>
                  <View style={styles.rangeBar}>
                    <View style={[styles.rangeFill, { width: '70%' }]} />
                    <View style={[styles.rangePointer, { left: '65%' }]} />
                  </View>
                  <View style={styles.rangeLabels}>
                    <ThemedText style={styles.rangeLow}>{'\u20B9'}{formatInr(g.low)}</ThemedText>
                    <ThemedText style={[styles.rangeChange, rateChange > 0 ? styles.rangeUp : styles.rangeDown]}>
                      {rateChange > 0 ? '\u25B2' : '\u25BC'} {rateChangePercent}% range
                    </ThemedText>
                    <ThemedText style={styles.rangeHigh}>{'\u20B9'}{formatInr(g.high)}</ThemedText>
                  </View>
                </View>
              ...
              <View style={[styles.calcCard, Shadow.sm]}>
                <View style={styles.calcHeader}>
                  <ThemedText style={styles.calcTitle}>Quick Estimate</ThemedText>
                  <View style={styles.gramSelector}>
                    {QUICK_AMOUNTS.map((amt) => (
                      <Pressable
                        key={amt}
                        onPress={() => setSelectedGrams(amt)}
                        style={[styles.gramChip, selectedGrams === amt && styles.gramChipActive]}>
                        <ThemedText style={[styles.gramText, selectedGrams === amt && styles.gramTextActive]}>
                          {amt}g
                        </ThemedText>
                      </Pressable>
                    ))}
                ...
                <View style={styles.calcResult}>
                  <View style={styles.calcRow}>
                    <ThemedText style={styles.calcLabel}>Gold 22K</ThemedText>
                    <ThemedText style={styles.calcValue}>
                      {'\u20B9'}{formatInr((g.rate22kt * selectedGrams) / 10)}
                    </ThemedText>
                  </View>
                  <View style={styles.calcDivider} />
                  <View style={styles.calcRow}>
                    <ThemedText style={styles.calcLabel}>Gold 24K</ThemedText>
                    <ThemedText style={styles.calcValue}>
                      {'\u20B9'}{formatInr((g.rate24kt * selectedGrams) / 10)}
                    </ThemedText>
                  </View>
                  {v18 && (
                    <>
                      <View style={styles.calcDivider} />
                      <View style={styles.calcRow}>
                        <ThemedText style={styles.calcLabel}>Gold 18K</ThemedText>
                        <ThemedText style={styles.calcValue}>
                          {'\u20B9'}{formatInr((v18.rate * selectedGrams) / 10)}
                        </ThemedText>
                      </View>
                    </>
                  )}
                ...
                <View style={styles.metaRow}>
                  ...
                  <View style={styles.confidenceRow}>
                    <View style={styles.confidenceBar}>
                      <View style={[styles.confidenceFill, { width: `${g.confidence}%` }]} />
                    </View>
...
const styles = StyleSheet.create({
  ...
  loader: { marginTop: 40 },
  errorCard: {
    backgroundColor: '#FEF2F2', borderRadius: BorderRadius.lg, padding: 16,
  ...
  /* Hero Card */
  heroCard: { backgroundColor: '#23519D', borderRadius: BorderRadius.xl, padding: 24, gap: 4 },
  ...
  heroBadge: { backgroundColor: 'rgba(201,168,76,0.2)', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  /* Range Bar */
  rangeContainer: { gap: 6 },
  rangeBar: { height: 6, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 3, position: 'relative' },
  rangeFill: { position: 'absolute', top: 0, left: 0, height: '100%', backgroundColor: 'rgba(201,168,76,0.4)', borderRadius: 3 },
  rangePointer: { position: 'absolute', top: -3, width: 12, height: 12, borderRadius: 6, backgroundColor: '#C9A84C', borderWidth: 2, borderColor: '#FFFFFF' },
  rangeLabels: { flexDirection: 'row', justifyContent: 'space-between' },
  rangeLow: { fontSize: 10, color: 'rgba(255,255,255,0.5)' },
  rangeHigh: { fontSize: 10, color: 'rgba(255,255,255,0.5)' },
  rangeChange: { fontSize: 10, fontWeight: '600' },
  rangeUp: { color: '#86EFAC' },
  rangeDown: { color: '#FCA5A5' },

  heroDivider: { height: 1, backgroundColor: 'rgba(201,168,76,0.25)', marginBottom: 8, marginTop: 8 },
  ...
  calcCard: {
    ...
    borderWidth: 1, borderColor: '#E5E1D8', gap: 12,
  },
  calcHeader: { gap: 10 },
  calcTitle: { fontSize: 15, fontWeight: '700', color: '#1A1A2E' },
  gramSelector: { flexDirection: 'row', gap: 8 },
  gramChip: {
    paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16,
    backgroundColor: '#F8F6F3', borderWidth: 1, borderColor: '#E5E1D8',
  },
  gramChipActive: { backgroundColor: '#23519D', borderColor: '#23519D' },
  gramText: { fontSize: 13, fontWeight: '600', color: '#6B7280' },
  gramTextActive: { color: '#FFFFFF' },
  calcResult: { gap: 8 },
  ...
  calcDivider: { height: 1, backgroundColor: '#F0ECE4' },

  /* Rate Grid */
  rateGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12 },
  tile: {
    width: '47%', flexGrow: 1, backgroundColor: '#FFFFFF', borderRadius: BorderRadius.lg,
    padding: 16, gap: 4, borderWidth: 1, borderColor: '#E5E1D8', overflow: 'hidden',
  },
  ...
  metaCard: {
    backgroundColor: '#FFFFFF', borderRadius: BorderRadius.lg, padding: 16,
    borderWidth: 1, borderColor: '#E5E1D8', gap: 10,
  },
  ...
  confidenceBar: { width: 60, height: 4, backgroundColor: '#F0ECE4', borderRadius: 2 },
  confidenceFill: { height: '100%', backgroundColor: '#16A34A', borderRadius: 2 },

  /* Alert Card */
  alertCard: {
    backgroundColor: '#FDF8ED', borderRadius: BorderRadius.lg, padding: 16,
    borderWidth: 1, borderColor: '#E8D9A8', gap: 8,
  },
  alertTitle: { fontSize: 15, fontWeight: '700', color: '#1A1A2E' },
  alertDesc: { fontSize: 13, color: '#6B7280', lineHeight: 18 },
  alertBtn: {
    backgroundColor: '#23519D', borderRadius: 10, paddingVertical: 10,
  ...
  /* Disclaimer */
  disclaimer: {
    backgroundColor: '#F8F6F3', borderRadius: BorderRadius.md, padding: 14,
