// Ported from payment-auditor/frontend/src/pages/BankActivityPage.tsx —
// same component, same behavior. Only change: LAN-implicit trust is gone,
// so every mutating call now goes through the shared api/client.ts (which
// attaches X-Notifier-Token), including the three calls (reference-copied,
// correction, test-popup) that used to skip getHeaders() entirely.
...
interface DocumentFile { filename: string; document_type: string | null; }
interface DashboardDocument { bundle_id: string; customer_name: string; document_type: string; document_count: number; status: string; created_at: string; files: DocumentFile[]; }
