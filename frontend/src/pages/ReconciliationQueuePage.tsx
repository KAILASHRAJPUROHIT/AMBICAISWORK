...
const normalizePaymentBreakdown = (value: unknown): ReconciliationPaymentEvidence[] => {
  ...
  return value.map((payment: any) => ({
    ...
    proofExists: Boolean(payment.proof_exists || payment.proofExists),
...
const ReconciliationQueuePage: React.FC = () => {
  ...
  const fetchReviews = useCallback(() => {
    setLoading(true);
    setError(null);
    ensureAccountantVerificationItems()
      .catch(err => {
        console.warn('Failed to ensure accountant verification items:', err);
        // Continue even if ensure items fails
      })
      .finally(() => {
  ...
  const handleAction = (type: 'APPROVE' | 'REJECT' | 'FURTHER_REVIEW', queueId: number | string) => {
  ...
  const submitAction = async () => {
    ...
    try {
      if (actionModal.type === 'APPROVE') {
        await approveAccountantVerification(Number(actionModal.queueId), actionNote);
      } else if (actionModal.type === 'REJECT') {
        if (!actionNote.trim()) throw new Error('Note is required for rejection');
        await rejectAccountantVerification(Number(actionModal.queueId), actionNote);
      } else {
        if (!actionNote.trim()) throw new Error('Note is required for further review');
        await furtherReviewAccountantVerification(Number(actionModal.queueId), actionNote, deferHours, deferHours);
      }
      setActionModal(null);
